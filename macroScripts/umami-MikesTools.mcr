...
function myRandom = ( -- randomizing selected keys
	undo on
	(
...
undo on (
			...
			for i = 1 to selection.count do (
				selection[i].pos.controller = copy mySpringPosController 
				
				)
		)
)

function makeSceneParent = ( -- a setter function to make a parent for the entire scene
	myPoint = Point()
	myPoint.pos = objects.center
	myPoint.Box = on
	myPoint.name = maxFileName +"parent"

	sizeArray = #()
	myPnt3 = objects.max - objects.min
	for i = 1 to 3 do(
		append sizeArray myPnt3[i]
		
	)
	
	mySize = amax sizeArray
...
rollout randomKeymov_Rollout "Random Key Mover" width:162 height:300(
	...
	( undo on 
		...
		for i = 2 to myArray.count do
			...
			myobj = myArray[i]
			mycount= far_time
			movekeys myobj  ((i-1) * mycount) #selection
			)
		)
	)
)
...
mikeTools_floater = newRolloutFloater "MikesTools" 265 600
			addRollout spring_rollout mikeTools_floater rolledUp:true
			addRollout randomKeymov_Rollout mikeTools_floater rolledUp:true
			addRollout fade_rollout mikeTools_floater rolledUp:true
			addRollout simpleBtnRoll mikeTools_floater rolledUp:true
			addRollout volselRoll mikeTools_floater rolledUp:true
